...
function buttonizeContinue() {
introContinueBut.interactive = true;
introContinueBut.buttonMode = true;
introContinueBut
    .on('mouseover', overfunContinue)
    .on('mouseup', releasedfunContinue)
    .on('mousedown', pressedfunContinue)
    .on('touchstart',pressedfunContinue)
    .on('touchend', releasedfunContinue)
    .on('mouseout', releasedfunContinue)
    .on('touchendoutside',releasedfunContinue);
}

function pressedfunContinue() {
introContinueBut.texture = id["continueButClicked.png"];
if (!buttonState.continue) {
  stateVar.continue.state = true;
}

  // console.log("pressed "+ buttonState[this.txt]);
}
function releasedfunContinue() {
introContinueBut.texture = id["continueButOff.png"];
buttonState.countStart = false;
    // console.log("released" + buttonState[this.txt]);
}

function overfunContinue() {
  introContinueBut.texture = id["continueButHover.png"];
    // this.defaultCursor = 'pointer';
    // console.log("I'm over "+buttonState[this.txt]);
}
